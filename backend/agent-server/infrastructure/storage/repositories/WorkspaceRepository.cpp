...
std::optional<WorkspaceRecord> WorkspaceRepository::findById(const std::string& workspace_id) {
    sqlite3_stmt* stmt = nullptr;
    const char* sql = "SELECT id, name, path, created_at FROM workspaces WHERE id = ?;";
    ...
    const auto* path = reinterpret_cast<const char*>(sqlite3_column_text(stmt, 2));
...
std::vector<WorkspaceRecord> WorkspaceRepository::listAll() {
    sqlite3_stmt* stmt = nullptr;
    const char* sql = "SELECT id, name, path, created_at FROM workspaces ORDER BY created_at DESC;";
    if (sqlite3_prepare_v2(db_, sql, -1, &stmt, nullptr) != SQLITE_OK) {
        throw std::runtime_error(lastError());
    }
