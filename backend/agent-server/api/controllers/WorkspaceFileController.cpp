...
std::string request_target(const std::string& request) {
    ...
    return request_line.substr(target_start, target_end - target_start);
}
...
std::string extract_query_string(const std::string& request, const std::string& key) {
    ...
    while (pos < target.size()) {
        ...
        const std::string part = target.substr(pos, next == std::string::npos ? std::string::npos : next - pos);
        const std::size_t equals = part.find('=');
        const std::string name = url_decode(equals == std::string::npos ? part : part.substr(0, equals));
        if (name == key) {
            return url_decode(equals == std::string::npos ? "" : part.substr(equals + 1));
        }
...
std::string WorkspaceFileController::getTree(const std::string& request) {
    ...
    try {
        ...
        if (!record) {
            sqlite3_close(db);
            return error_response("WORKSPACE_NOT_FOUND", "workspace not found", "404 Not Found");
        }
        ...
        sqlite3_close(db);
        return error_response("DATABASE_ERROR", error.what(), "500 Internal Server Error");
    }
}

std::string WorkspaceFileController::getFileContent(const std::string& request) {
    const std::string workspace_id = extract_workspace_id(request);
    const std::string relative_path = extract_query_string(request, "path");
    if (workspace_id.empty() || relative_path.empty()) {
        return error_response("INVALID_REQUEST", "workspace_id and path are required");
    }
    ...
    try {
        ...
        if (size > kMaxReadableFileSize) {
            sqlite3_close(db);
            return error_response("FILE_TOO_LARGE", "file exceeds maximum readable size", "413 Payload Too Large");
        }
        if (workspace.isBinaryFile(relative_path)) {
            sqlite3_close(db);
            return error_response("BINARY_FILE", "binary files cannot be read", "415 Unsupported Media Type");
        }
