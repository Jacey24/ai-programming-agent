...
std::string extract_workspace_root(const std::string& config_path) {
    ...
    const std::size_t root_pos = content.find(root_marker, workspace_pos + workspace_marker.size());
    if (root_pos == std::string::npos) {
        return "/workspace";
    }

    const std::size_t colon_pos = content.find(':', root_pos + root_marker.size());
    const std::size_t first_quote = content.find('"', colon_pos);
    const std::size_t second_quote = content.find('"', first_quote + 1);
    if (colon_pos == std::string::npos || first_quote == std::string::npos || second_quote == std::string::npos) {
        return "/workspace";
...
int run_agent_server(const std::string& config_path) {
    ...
    const bool database_connected = initialize_database(database_path, database_error);
    codepilot::ToolSystem::getInstance().init(workspace_root, config_path);

    std::cout << "CodePilot Agent Server starting\n";
    std::cout << "Config: " << config_path << "\n";
    std::cout << "Workspace: " << workspace_root << "\n";
    std::cout << "SQLite: " << database_path << " connected=" << (database_connected ? "true" : "false") << "\n";
    if (!database_error.empty()) {
        std::cerr << "SQLite error: " << database_error << "\n";
    }
