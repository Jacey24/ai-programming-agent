...
function dedupeToolEvents(events: TaskEventRecord[]): TaskEventRecord[] {
  ...
  events.forEach((evt, idx) => {
    ...
    const key = TOOL_EVENT_TYPES.includes(type)
      ? strMeta(m, 'tool_call_id') || strMeta(m, 'tool_name') || `tool_${idx}`
      : strMeta(m, 'permission_id') || strMeta(m, 'request_id') || strMeta(m, 'tool_name') || `perm_${idx}`;
    latest.set(key, idx);
  });
...
export function ChatView({ workspaceId, session, onBack }: Props) {
  ...
  useEffect(() => {
    ...
    const loadHistory = async () => {
      ...
      try {
        ...
        const tasks = (taskRes.items || []).sort((a, b) =>
          (a.created_at || '').localeCompare(b.created_at || ''));
        const historyEvents: TaskEventRecord[] = [];

        for (const t of tasks) {
          if (t.goal) {
            addMessage({ id: `user_${t.id}`, role: 'user', content: t.goal, timestamp: t.created_at || '' });
          }
          try {
            const evtRes = await listTaskEvents(t.id);
            for (const evt of evtRes.items || []) {
              const channel = strMeta(objMeta(evt.metadata), 'channel');
              if (channel === 'dialog' && evt.content && evt.type === 'agent_message') {
                addMessage({
                  id: evt.id || `evt_${t.id}_${evt.created_at}`,
                  role: 'assistant',
                  content: evt.content,
                  expert: strMeta(objMeta(evt.metadata), 'expert'),
              ...
              // Convert tool debug messages to tool_finished events
              if (evt.type === 'agent_message' && channel === 'debug' && evt.content) {
                const meta = objMeta(evt.metadata);
                const src = strMeta(meta, 'source');
                const toolName = strMeta(meta, 'tool_name');
                if (src === 'tool' && toolName) {
                  const content = evt.content || '';
                  const success = !content.includes('[EXECUTION_ERROR]') && !content.includes('异常:');
                  historyEvents.push({
                    ...evt, type: 'tool_finished',
                    metadata: { tool_name: toolName, success },
                  } as TaskEventRecord);
                }
              }
            }
        ...
        if (historyEvents.length > 0) appendEvents(historyEvents);
      } catch { /* empty */ }
    ...
    setRawEvents([]);
    loadHistory();
  }, [session.id, addMessage, appendEvents, setRawEvents]);
  ...
  const handleCancel = async () => {
    ...
    try { await cancelTask(activeTask.id); closeSSE(); } catch { /* empty */ }
  };

  const handleApprove = useCallback(async (permId: string) => {
    if (!permId) return;
    try { await approvePermission(permId); setResolvedPerms(prev => new Set(prev).add(permId)); } catch { /* empty */ }
  }, []);

  const handleReject = useCallback(async (permId: string) => {
    if (!permId) return;
    try { await rejectPermission(permId); setResolvedPerms(prev => new Set(prev).add(permId)); } catch { /* empty */ }
  }, []);

  // Build unified timeline
  const unifiedTimeline: TimelineItem[] = useMemo(() => {
    if (loadingHistory) return [{ kind: 'loading' }];
    ...
    items.sort((a, b) => {
      if ('time' in a && 'time' in b) return a.time.localeCompare(b.time);
      return 0;
    });
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center shrink-0 border-b border-[var(--glass-border-strong)]"
        style={{ height: 44, padding: '0 20px', gap: 8 }}>
        <button onClick={onBack} className="btn-secondary"
          style={{ minWidth: 56, height: 30, fontSize: 12, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '0 12px' }}>
          ← 返回
        </button>
        <span className="text-xs text-[var(--text-primary)] truncate flex-1">{session.title || '对话'}</span>
        {streaming && <span className="w-2 h-2 rounded-full bg-[var(--accent-light)] animate-pulse" style={{ animation: 'pulse 1.5s infinite' }} />}
      </div>
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto"
        ...
        {unifiedTimeline.map(item => {
          ...
          if (item.kind === 'streaming') {
            return (
              <div key="streaming-bubble" className="flex justify-start">
                <div className="max-w-[85%] rounded-xl text-xs leading-relaxed"
                  style={{ padding: '8px 12px', background: 'var(--bubble-ai)', border: '1px solid var(--glass-border)', color: 'var(--text-primary)' }}>
                  <div className="markdown-body">
                    <ReactMarkdown>{item.data.content}</ReactMarkdown>
                    <span style={{ display: 'inline-block', width: 1, height: 14, background: 'var(--accent-light)', marginLeft: 1, verticalAlign: 'text-bottom', animation: 'blink 1s step-end infinite' }} />
                  </div>
                </div>
              </div>
            );
          ...
          if (item.kind === 'dot-loader') {
            return (
              <div key="dot-loader" className="flex justify-start">
                <div className="rounded-xl bg-[var(--bubble-ai)] border border-[var(--glass-border)]"
                  style={{ padding: '8px 12px', display: 'inline-flex', gap: 4 }}>
          ...
          if (item.kind === 'perm') {
            ...
            return (
              <div key={item.data.id} className="flex justify-start">
                <div className="rounded-xl border flex flex-col gap-2"
                  style={{ padding: '10px 14px', maxWidth: '85%', background: 'rgba(59,130,246,0.08)', borderColor: 'rgba(59,130,246,0.35)' }}>
                  <div className="flex items-start gap-2">
                    <span style={{ width: 8, height: 8, borderRadius: '50%', background: '#3b82f6', flexShrink: 0, marginTop: 2, animation: 'pulse 1.5s infinite' }} />
                  ...
                  {!isResolved && item.data.permissionId && (
                    <div className="flex gap-2" style={{ paddingLeft: 16 }}>
                      <button onClick={() => handleApprove(item.data.permissionId)} className="btn-primary"
                        style={{ height: 26, fontSize: 11, padding: '0 14px', minWidth: 52, borderRadius: 6 }}>批准</button>
                      <button onClick={() => handleReject(item.data.permissionId)} className="btn-danger"
                        style={{ height: 26, fontSize: 11, padding: '0 14px', minWidth: 52, borderRadius: 6 }}>拒绝</button>
                    </div>
                  )}
                  {isResolved && (
          ...
          if (item.kind === 'status') {
            const statusColors = {
              running: { bg: 'rgba(59,130,246,0.1)', border: 'rgba(59,130,246,0.3)', dot: '#3b82f6' },
              success: { bg: 'rgba(34,197,94,0.1)', border: 'rgba(34,197,94,0.3)', dot: '#22c55e' },
              failed: { bg: 'rgba(239,68,68,0.1)', border: 'rgba(239,68,68,0.3)', dot: '#ef4444' },
            };
            const c = statusColors[item.data.status];
            return (
              <div key={item.data.id} className="flex justify-start">
                <div className="rounded-xl border flex items-start gap-2"
                  style={{ padding: '8px 12px', background: c.bg, borderColor: c.border }}>
                  <span style={{ width: 8, height: 8, borderRadius: '50%', background: c.dot, flexShrink: 0, marginTop: 2,
