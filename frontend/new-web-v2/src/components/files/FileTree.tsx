  ...
  for (const item of items) {
    ...
    parts.forEach((part, index) => {
      ...
      const node: TreeNode = {
        ...
        type: leaf ? item.type : 'directory', size: leaf ? item.size : 0,
        children: [],
      };
...
export function FileTree({ workspace, theme }: Props) {
  ...
  const loadRoot = useCallback(async () => {
    ...
    try {
      ...
      const raw = res as unknown as { items?: FlatEntry[] };
      if (Array.isArray(raw.items)) setRootNodes(buildTree(raw.items));
      else setRootNodes([]);
    } catch { setRootNodes([]); }
    setLoading(false);
  }, [workspace?.id]);
  ...
  const openPreview = useCallback(async (filePath: string, rowEl: HTMLButtonElement) => {
    ...
    try {
      ...
      setPreview(prev => prev?.filePath === filePath
        ? { ...prev, content: res.content || '', loading: false } : prev);
    } catch {
      setPreview(prev => prev?.filePath === filePath
        ? { ...prev, error: '加载失败', loading: false } : prev);
    }
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto" ref={listRef} style={{ padding: '8px 0' }}>
        {loading ? (
          ...
          rootNodes.map(node => (
            <FileTreeNode key={node.path} node={node} depth={0} expanded={expanded}
              onToggle={toggleDir} onPreview={openPreview}
              activePath={preview?.filePath ?? null} />
          ))
        )}
      </div>
        ...
        <div className={`theme-${theme} glass-panel flex flex-col overflow-hidden`} style={popStyle}>
          <div className="flex items-center justify-between shrink-0 border-b border-[var(--glass-border-strong)]"
            ...
            <button onClick={closePreview}
              style={{ width: 24, height: 24, borderRadius: '50%', flexShrink: 0, border: '1px solid var(--glass-border-strong)', background: 'var(--surface)', color: 'var(--text-secondary)', cursor: 'pointer', fontSize: 12, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>✕</button>
          </div>
