...
const LEVEL_FULL = 220;
const LEVEL_COMPACT = 140;
const LEVEL_MINIMAL = 75;
...
export function StatusPills() {
  ...
  const pills: PillStatus[] = useMemo(() => {
    ...
    return [
      ...
      {
        label: 'Database', value: !health ? 'checking' : dbOk ? 'connected' : 'disconnected',
        detail: health?.database?.path || health?.database?.type || (healthError ? 'health check failed' : 'waiting'),
        tone: dbStatus,
      },
      {
        label: 'LLM', value: health?.llm ? health.llm.model : '--',
        detail: health?.llm?.provider || 'check config',
        tone: health?.llm ? 'ok' : 'idle',
      },
      {
        label: 'Tasks', value: taskCount > 0 ? `${taskCount} active` : 'idle',
  ...
  return (
    <div ref={containerRef} style={{
      ...
      border: '1.5px solid var(--glass-border-strong)', borderRadius: 999,
      height: 40, maxWidth: '100%', padding: '0 12px',
      boxShadow: 'inset 0 1px 0 rgba(255,255,255,0.05), 0 4px 16px var(--shadow)',
    }}>
      {pills.map((pill, idx) => (
        <div key={pill.label} style={{ flex: '1 1 0', minWidth: 0, display: 'flex', alignItems: 'center', justifyContent: 'flex-start', gap: 0 }}>
          {idx > 0 && (
            <div style={{ width: 1, height: 22, background: 'var(--glass-border-strong)', marginRight: 12, flexShrink: 0 }} />
          )}
          <span style={{ width: 8, height: 8, borderRadius: '50%', background: toneColors[pill.tone], boxShadow: `0 0 6px ${toneColors[pill.tone]}44`, flexShrink: 0, marginRight: 8 }} />
          {level === 'full' && (
            <span style={{ display: 'flex', alignItems: 'baseline', gap: 6, minWidth: 0, overflow: 'hidden' }}>
              <span style={{ fontSize: 11, fontWeight: 600, color: 'var(--text-secondary)', whiteSpace: 'nowrap', flexShrink: 0 }}>{pill.label}</span>
              <span style={{ fontSize: 11, fontWeight: 700, whiteSpace: 'nowrap', flexShrink: 0, color: toneColors[pill.tone] }}>{pill.value}</span>
              <span style={{ fontSize: 10, fontWeight: 400, color: 'var(--text-secondary)', opacity: 0.7, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{pill.detail}</span>
            </span>
          )}
          {level === 'compact' && (
            <span style={{ display: 'flex', alignItems: 'baseline', gap: 6, minWidth: 0, overflow: 'hidden' }}>
              <span style={{ fontSize: 11, fontWeight: 600, color: 'var(--text-secondary)', whiteSpace: 'nowrap', flexShrink: 0 }}>{pill.label}</span>
              <span style={{ fontSize: 11, fontWeight: 700, whiteSpace: 'nowrap', color: toneColors[pill.tone], overflow: 'hidden', textOverflow: 'ellipsis' }}>{pill.value}</span>
            </span>
          )}
          {level === 'minimal' && (
            <span style={{ fontSize: 11, fontWeight: 700, whiteSpace: 'nowrap', color: toneColors[pill.tone], overflow: 'hidden', textOverflow: 'ellipsis' }}>{pill.value}</span>
