...
export function SessionList({ workspaceId, onSelect }: Props) {
  ...
  const refresh = useCallback(async () => {
    ...
    setLoading(true);
    try { const res = await listSessionsByWorkspace(workspaceId); setSessions(res.items || []); } catch { /* empty */ }
    setLoading(false);
  }, [workspaceId]);
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      setNewTitle(''); setShowNew(false); onSelect(s);
    } catch { alert('创建会话失败'); }
  };
  ...
  return (
    <div className="flex flex-col h-full" style={{ padding: 24, gap: 20 }}>
      ...
      {showNew && (
        <div className="flex anim-slide-down" style={{ gap: 8 }}>
          ...
          sessions.map(s => (
            <div key={s.id} className="flex items-center gap-1.5 group">
              ...
              <button
                ...
                style={{ width: 28, height: 28, flexShrink: 0, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14, color: 'var(--text-secondary)', background: 'transparent', border: '1px solid transparent', cursor: 'pointer' }}
