...
export function GlassPanel({
  ...
  return (
    <GlassWindow zIndex={10} position="relative" pointerEvents="auto" style={{
      ...
      {/* 浏览器风格 Tab 栏 — 返回按钮 + tab 并列 */}
      <div className="flex shrink-0 items-end" style={{ padding: `${16 / scale}px ${16 / scale}px 0 ${16 / scale}px`, gap: `${2 / scale}px` }}>
        {/* 返回按钮 — 浏览器风格的 ← */}
        <button
        ...
        <div
          ...
          style={{
            flex: 1, height: 38 / scale, display: 'flex', alignItems: 'center', justifyContent: 'center',
            fontSize: 13 / scale, fontWeight: 600, letterSpacing: '0.02em',
      ...
      {/* 内容区 — 顶部用 border-top 衔接活跃 tab 下沿 */}
      <div key={activeTab} className="flex-1 min-h-0 flex flex-col overflow-hidden anim-fade-in" style={{ borderTop: '1px solid var(--glass-border-strong)' }}>
        {activeTab === 'chat' ? (
          activeSession ? (
            <ChatView
