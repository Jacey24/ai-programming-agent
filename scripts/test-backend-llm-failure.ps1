...
try{
  ...
  $script:workspaceId=(Call POST '/api/v1/workspaces' @{name='LLM failure regression';path='./workspace'}).data.id;$script:sessionId=(Call POST '/api/v1/sessions' @{title='LLM failure regression';workspace_id=$script:workspaceId}).data.id
