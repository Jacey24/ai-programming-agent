...
try {
  ...
  $script:workspaceId=(Call POST '/api/v1/workspaces' @{name='Cancellation regression';path='./workspace'}).data.id;$script:sessionId=(Call POST '/api/v1/sessions' @{title='Cancellation regression';workspace_id=$script:workspaceId}).data.id
